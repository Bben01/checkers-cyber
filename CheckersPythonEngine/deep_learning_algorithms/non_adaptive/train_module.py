...
def fight_agent(best_model: str, current_model: str, ae,
    ...
    """
    from deep_learning_algorithms.non_adaptive.util.reinforcement_algorithm import AlphaZeroAgent

    loss_win = {
        0: 0,
        1: 0
    }
    for tour in range(round_fight):
        print("ROUND {}".format(tour + 1))
        terminal = False
        count_turn = 1
        state = State()
        state.initial_state()
        ...
        while not terminal and count_turn <= max_turn:
            ...
            if count_turn > max_turn:
                print("ROUND {} DRAW".format(tour + 1))
    return loss_win
